...
exports.ClaudeProcessor = void 0;
...
class ClaudeProcessor {
    constructor() {
        ...
        if (!this.apiKey) {
            logger_1.logger.warn('ANTHROPIC_API_KEY not found - Claude processing will be disabled');
            return;
        }
    ...
    async generateSummary(text, maxLength = 150) {
        if (!this.apiKey || !text || !this.client) {
            return (0, helpers_1.truncateText)(text, maxLength);
        }
...
${text}`;
        try {
            ...
            const summary = response.content[0]?.type === 'text'
                ...
                : '';
            return (0, helpers_1.truncateText)(summary, maxLength);
        }
        catch (error) {
            logger_1.logger.error('Error generating summary with Claude:', error);
            if (error instanceof sdk_1.default.RateLimitError) {
                logger_1.logger.warn('Claude API rate limit exceeded - using fallback summary');
                await (0, helpers_1.delay)(10000); // Wait 10 seconds for rate limit
            }
            // Fallback to simple truncation
            return (0, helpers_1.truncateText)(text, maxLength);
        }
    }
    async categorizeContent(item) {
...
JSON response should look like:
{
  ...
  "suggested_short_summary": "A new paper explores transformer efficiency."
        ...
        try {
            ...
            logger_1.logger.error('Error categorizing content with Claude:', error);
            if (error instanceof sdk_1.default.RateLimitError) {
                logger_1.logger.warn('Claude API rate limit exceeded during categorization');
                await (0, helpers_1.delay)(15000); // Wait 15 seconds for rate limit
            }
    ...
    async batchProcess(items) {
        ...
        logger_1.logger.info(`🤖 Processing ${items.length} items with Claude AI...`);
        for (let i = 0; i < items.length; i += batchSize) {
            const batch = items.slice(i, i + batchSize);
            const batchPromises = batch.map(async (item, index) => {
                try {
                    // Add small delay to avoid overwhelming the API
                    await (0, helpers_1.delay)(index * 1000);
                    return await this.categorizeContent(item);
                }
                catch (error) {
                    logger_1.logger.error(`Error processing item ${i + index}:`, error);
                    return this.getFallbackProcessing(item);
                }
            });
            try {
        ...
        logger_1.logger.info(`✅ Completed processing ${processed.length} items`);
        return processed;
    }
    getContentForAnalysis(item) {
        ...
        switch (item.type) {
            ...
            case 'news':
                const news = item;
                return news.full_text_content || news.description || news.title;
            case 'video':
                const video = item;
                return video.description || video.title;
            case 'blog':
                const blog = item;
                return blog.full_text_content || blog.description || blog.title;
            default:
    ...
    parseClaudeResponse(response) {
        try {
            ...
            logger_1.logger.error('Error parsing Claude response:', error);
    ...
    extractFallbackSummary(item) {
        const content = this.getContentForAnalysis(item);
        return (0, helpers_1.truncateText)(content, 150);
    }
}
exports.ClaudeProcessor = ClaudeProcessor;
