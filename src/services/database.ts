...
export class DatabaseService {
  ...
  async updateBookmark(id: string, bookmarked: boolean): Promise<boolean> {
    return new Promise((resolve, reject) => {
      this.db.run(
        'UPDATE items SET bookmarked = ? WHERE id = ?',
        [bookmarked ? 1 : 0, id],
        function (err) {
          if (err) {
            reject(err);
  ...
  async getStats(): Promise<DatabaseStats> {
    return new Promise((resolve, reject) => {
      this.db.serialize(() => {
        ...
        // Bookmarked count
        this.db.get(
          'SELECT COUNT(*) as count FROM items WHERE bookmarked = 1',
          (err, result: any) => {
            if (err) {
              reject(err);
              return;
  ...
  async updateSourceMetadata(
    ...
    return new Promise((resolve, reject) => {
      ...
      `,
        [sourceId, lastFetchTime, lastItemId],
        err => {
          if (err) {
            reject(err);
          } else {
  ...
  async getSourceMetadata(
    sourceId: string
  ): Promise<{ lastFetchTime: string; lastItemId?: string } | null> {
    return new Promise((resolve, reject) => {
      this.db.get(
